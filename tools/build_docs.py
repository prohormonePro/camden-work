...
def build(base):
    ...
    (DOCS/'llms.txt').write_text('# Camden Work\n\n> Experimental offline work continuity for a bounded synthetic target.\n\nResearch definitions are data, not authority or proof of runtime mitigation. No remote execution service is hosted here.\n\n## Documentation\n\n'+''.join('- ['+name.replace('-',' ').title()+']('+base+name+'.md)\n' for name in ['positioning','evidence','operator-externalization','continuity-gauntlet','overview','architecture','constitution','quickstart','verification-2026-09-23','tools','catalog','limits','maintenance','security','contributing','sources','crosswalks','related-work','licensing']),encoding='utf8')
